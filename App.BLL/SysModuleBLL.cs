...
namespace App.BLL
{
    public class SysModuleBLL : IBLL.ISysModuleBLL
    {
        public SysMgr_DBEntities db
        {
            get
            {
        ...
        [Dependency]
        public IDAL.ISysModuleRepository m_Rep { get; set; }

        public List<SysModuleModel> GetList(string parentId)
        {
            IQueryable<SysModule> queryData = null;
            queryData = m_Rep.GetList().Where(a => a.ParentId == parentId).OrderBy(a => a.Sort);
